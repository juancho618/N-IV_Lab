...
def simulation():
    ...
    Q = Q_discrete_white_noise(dim=3, dt=dt, var=qf)
    R = np.random.normal(0, np.square(r), iterations)
    X = np.matrix([[0.0],[0.0], [0.0]]) # Initial X values
    P = np.linalg.inv(np.matrix([[ r, r/dt, 0], [r/dt, 2*r/(dt**2), 0], [0, 0, 1]]))
    A = np.matrix([[1., dt, dt**2/2], [0, 1., dt], [0, 0, 1.]]) # The state is a constant
    U = 0 # There is no control input.
    B = np.matrix([[0.0], [0.0], [0.0]]) # There is no control input
    C = np.matrix([[1, 0, 0]]) # The noisy measurement is direct
    B = G
    ...
    # pylab.title('Simulation results for qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r) )
    # pylab.savefig('./img/acc/qf'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')

    # pylab.figure()
    # pylab.plot(plist[1:],color='r',label='P11')
    # pylab.title('Estimated P Variance for the Position for qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r))
    # pylab.savefig('./img/acc/P11_qf'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')

    # pylab.figure()
    # pylab.plot(p_22[1:],color='r',label='P22')
    # pylab.title('Estimated P Variance for the Velocity for qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r))
    # pylab.savefig('./img/acc/P22_qf'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')



    # pylab.figure()
    # pylab.plot(k_dist[1:],color='g')
    # pylab.title('Kalman gain for distance with qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r) )
    # pylab.savefig('./img/acc/K11_qf'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')

    # pylab.figure()
    # pylab.plot(k_vel[1:],color='g')
    # pylab.title('Kalman gain for velocity with qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r) )
    # pylab.savefig('./img/acc/K22_qf'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')


    # pylab.figure()
    # pylab.plot(NEES[1:],color='y')
    # pylab.title('Normalized Estimation Error Squared qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r))
    # pylab.savefig('./img/acc/nees'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')


    # pylab.figure()
    # pylab.plot(NIS[1:],color='purple',label='estimate value')
    # pylab.title('Normalized Innovation Squared qf = ' + str(qf) + ' and qg =' + str(qg) + ' and  R =' + str(r))
    # pylab.savefig('./img/acc/nis'+str(qf)+'_qg'+str(qg)+'_r'+str(r)+'.png')
    ...
    return(NEES,NIS,Zs_list,qf,qg,r)
